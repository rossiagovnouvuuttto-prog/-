...
1. Получите ключ у своего сервиса.
2. На сайте под полем ввода нажмите **«Подключить свой AI»**.
3. Выберите сервис, вставьте ключ, нажмите **«Проверить и подключить»**.

Поддерживаются:

| Сервис | Формат API | Модель по умолчанию |
|---|---|---|
| GLM / Zhipu | OpenAI-совместимый | `glm-4.7-flash` (бесплатная) |
| Ollama — облако | Ollama | `gpt-oss:120b-cloud` |
| Ollama на компьютере | Ollama | `llama3.2`, ключ не нужен |
| Другой OpenAI-совместимый | OpenAI-совместимый | задаётся вручную |

Последний пункт покрывает почти всё: DeepSeek, OpenAI, OpenRouter, локальные
серверы — достаточно указать адрес вида `https://api.example.com/v1`.
