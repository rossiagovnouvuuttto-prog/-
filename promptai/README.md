...
│   └── ollama.mjs              # клиент Ollama и промпты для модели
├── tests/
...
и без ключей. Чтобы промпты писала настоящая модель, поднимите сервер: он
держит ключ у себя и не отдаёт его в браузер.

### Шаг 1. Получить ключ

1. Откройте <https://ollama.com/settings/keys>.
2. Нажмите **Add API Key** и скопируйте ключ. Показывается он один раз.

Ключ — это доступ к вашему платному аккаунту. Никому его не передавайте, не
вставляйте в код страницы и не публикуйте: всё, что попало в `assets/`, видно
любому посетителю сайта.

### Шаг 2. Положить ключ рядом с сайтом
...
Файл `.env` уже внесён в `.gitignore` — в репозиторий он не попадёт.

### Шаг 3. Запустить

```bash
cd promptai
npm start          # то же самое: node server/server.mjs
```

Откройте <http://localhost:4173>. При старте сервер сам проверит связь и
напишет, подключилась модель или нет. Если да, под полем ввода появится
отметка «AI подключён» с названием модели.
