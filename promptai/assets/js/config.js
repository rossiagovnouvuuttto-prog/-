 ...
 *
 * provider: 'auto'  — сайт сам проверяет, поднят ли бэкенд с настоящей моделью,
 *                     и если нет, считает промпты встроенным движком;
 *           'local' — всегда встроенный движок (работает без сети);
 *           'api'   — всегда бэкенд (см. assets/js/ai/api-provider.js).
 *
 * Ключи моделей держите на сервере: браузер обращается только к своему
 * эндпоинту и ключей не видит.
 */
export const AI = {
  provider: 'auto',
  endpoint: '/api/prompt',
  /** Проверка доступности бэкенда для режима 'auto'. */
