...
- **Пять стилей оформления** — Pokémon, Аниме, Minecraft, Roblox, GTA SA.
  ...
  скругления и логотип, а Minecraft ещё и шрифт на моноширинный. У четырёх
  стилей логотип — своя картинка, она же показана образцом в «Настройках»
...
Картинки-логотипы лежат в `static/themes/*.png` квадратами 160×160. В конец
`styles.css` они вшиты как `data:`-строки — так их подхватывают все три сборки
сразу, без отдельной раздачи файлов. Поменяли картинку — пересоберите блок:
