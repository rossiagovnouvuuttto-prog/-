...
The Ollama key lives ONLY here, in the OLLAMA_API_KEY environment variable.
It is never sent to the browser: the frontend talks to /api/chat and this
process adds the Authorization header on its way out to Ollama.
"""
...
OLLAMA_BASE_URL = (os.environ.get("OLLAMA_BASE_URL") or "https://ollama.com/v1").rstrip("/")
...
# Error translation
# --------------------------------------------------------------------------- #
MODEL_UNAVAILABLE = "Эта модель сейчас недоступна в Ollama. Выберите другую."


def classify_http_error(status: int, body: str) -> ChatError:
    """Map an Ollama response onto a friendly Russian message."""
    low = (body or "").lower()

    if status in (401, 403):
        return ChatError(
            "bad_token",
            "Неверный ключ Ollama. Проверьте его в настройках.",
            status,
        )
    if status == 402:
        return ChatError(
            "quota",
            "Исчерпан лимит Ollama на этом аккаунте. Проверьте раздел Usage на ollama.com.",
            status,
        )
    if status == 404:
    ...
    if status == 429:
        return ChatError(
            "rate_limit",
            "Превышен лимит запросов Ollama. Подождите немного и повторите.",
            status,
        )
    if status == 503 or "loading" in low:
        return ChatError(
            "model_loading",
            "Модель загружается на стороне Ollama. Попробуйте ещё раз через полминуты.",
            status,
        )

    detail = extract_error_message(body)
    suffix = f" ({detail})" if detail else ""
    return ChatError("api_error", f"Ошибка Ollama {status}{suffix}", status)
...
def validate(req: ChatRequest) -> None:
    if not OLLAMA_KEY:
        raise ChatError(
            "no_token",
            "На сервере не задан ключ Ollama (OLLAMA_API_KEY). "
            "Добавьте его и перезапустите приложение.",
            503,
        )
    if not MODEL_ID_RE.match(req.model):
        raise ChatError("bad_model_id", "Некорректное имя модели. Формат: имя:тег", 400)
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            async with client.stream(
                ...
                if resp.status_code >= 400:
                    ...
                    log.warning("Ollama %s for %s: %s", resp.status_code, req.model, body[:300])
                    yield sse({"type": "error", "code": err.code, "message": err.message})
                    return
                ...
                async for raw in resp.aiter_lines():
                    ...
                    if isinstance(chunk, dict) and chunk.get("error"):
                        msg = extract_error_message(data) or MODEL_UNAVAILABLE
                        yield sse({"type": "error", "code": "api_error", "message": msg})
                        return
        ...
        yield sse(
            {
                "type": "error",
                "code": "network",
                "message": "Не удалось связаться с Ollama.",
            }
        )
...
async def complete_once(req: ChatRequest) -> dict[str, Any]:
    ...
    try:
        ...
        raise ChatError("network", "Не удалось связаться с Ollama.", 502)
...
async def fetch_served_ids(client: httpx.AsyncClient) -> set[str] | None:
    """Model names the account can use, or None if the listing failed."""
    try:
...
async def resolve_featured(force: bool = False) -> list[dict[str, Any]]:
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        ...
        async def resolve_one(entry: dict[str, Any]) -> dict[str, Any]:
            ...
            if served:
                chosen = next((c for c in cands if c.strip().lower() in served), None)
                if chosen:
                ...
                if entry.get("fallbackToListing"):
                    return card(entry, sorted(served)[0], "online")
...
async def health() -> dict[str, Any]:
    ...
    return {
        "ok": True,
        "token_configured": bool(OLLAMA_KEY),
